import { serial, integer, pgEnum, pgTable, text, timestamp, varchar, boolean, json, numeric } from "drizzle-orm/postgresql-core";
...
export const agents = pgTable("agents", {
  ...
  status: pgEnum("status", ["idle", "running", "error", "stopped"]).default("idle").notNull(),
...
export const auditLogs = pgTable("audit_logs", {
  ...
  category: pgEnum("category", ["agent", "skill", "auth", "system", "task", "file"]).default("system").notNull(),
  severity: pgEnum("severity", ["info", "warning", "error", "critical"]).default("info").notNull(),
...
export const scheduledTasks = pgTable("scheduled_tasks", {
  ...
  taskType: pgEnum("taskType", ["cron", "interval", "once"]).default("cron").notNull(),
  prompt: text("prompt"),
  enabled: boolean("enabled").default(true),
  lastRunAt: timestamp("lastRunAt"),
  nextRunAt: timestamp("nextRunAt"),
  lastStatus: pgEnum("lastStatus", ["pending", "running", "success", "failed"]).default("pending"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
export const agentFiles = pgTable("agent_files", {
  ...
  category: pgEnum("category", ["artifact", "log", "config", "other"]).default("other").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
export const integrations = pgTable("integrations", {
  ...
  status: pgEnum("status", ["active", "expired", "revoked"]).default("active").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
export const llmConfigs = pgTable("llm_configs", {
  ...
  agentId: integer("agentId"),
  name: varchar("name", { length: 255 }).notNull(),
  provider: pgEnum("provider", ["openai", "anthropic", "custom", "manus"]).notNull(),
  ...
  status: pgEnum("status", ["active", "inactive", "error"]).default("active").notNull(),
...
export const serviceIntegrations = pgTable("service_integrations", {
  ...
  agentId: integer("agentId"),
  name: varchar("name", { length: 255 }).notNull(),
  service: pgEnum("service", ["slack", "discord", "github", "webhook", "custom"]).notNull(),
